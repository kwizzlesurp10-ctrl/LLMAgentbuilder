...
class AgentEngine:
    ...
    def execute(
        ...
        try:
            ...
            # Note: We execute the agent code directly, even if Copilot token is detected.
            # The generated agent code itself uses CopilotClient internally, so we should
            # load and execute the agent class normally, not bypass it.
    ...
    def execute_with_timeout(
        ...
        try:
            ...
            # Note: We execute the agent code directly via subprocess, even if Copilot token is detected.
            # The generated agent code itself uses CopilotClient internally, so we should
            # execute the agent script normally, not bypass it.
            
            # Determine if agent_source is a file path or code string
            # Code strings typically contain newlines and class definitions
            is_code_string = (
                isinstance(agent_source, str) and 
                ('\n' in agent_source or 'class ' in agent_source or 'import ' in agent_source)
            )
            
            if isinstance(agent_source, Path) or (isinstance(agent_source, str) and not is_code_string):
                # Treat as file path
    ...
    def _get_env_with_api_key(self) -> Dict[str, str]:
        ...
        # Ensure Python path includes the project root so subprocess can import llm_agent_builder
        # Get the directory containing llm_agent_builder package (project root)
        current_file = os.path.abspath(__file__)
        # agent_engine.py is in llm_agent_builder/, so go up 2 levels to get project root
