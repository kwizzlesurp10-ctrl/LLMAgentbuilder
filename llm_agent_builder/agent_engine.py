...
class AgentEngine:
    ...
    def execute_with_timeout(
        ...
        try:
            ...
            if is_file:
                ...
                return self._execute_subprocess(agent_file, task, start_time)
            else:
                # Create temporary file from code string and execute within context
                with temporary_python_file(str(normalized_source)) as temp_path:
                    return self._execute_subprocess(temp_path, task, start_time)
    ...
    def _execute_subprocess(self, agent_file: str, task: str, start_time: float) -> ExecutionResult:
        """
        Execute an agent file via subprocess with timeout.

        :param agent_file: Path to the agent file to execute
