...
Examples:
  ...
  llm-agent-builder batch agents.json
    ...
    gen_parser.add_argument(
        "--provider", default="anthropic", choices=["anthropic", "huggingface"], help="LLM Provider to use"
    )
    ...
    try:
        if args.command == "generate":
            ...
            default_model = model or (
                "claude-3-5-sonnet-20241022" if provider == "anthropic" else "meta-llama/Meta-Llama-3-8B-Instruct"
            )
            agent_code = builder.build_agent(
                agent_name=name,
                prompt=prompt,
