...
def main() -> None:
    ...
    # CLI subcommands (we'll check if any of these are present)
    parser.add_argument("command", nargs="?", help="CLI command (generate, list, test, batch)")
    
    # Try to parse known args first
    ...
    is_cli_command = args.command in cli_commands or args.cli
    
    # Mode routing logic:
    # 1. If --serve flag is used OR no arguments provided -> Web mode
    # 2. If CLI command is provided OR --cli flag is used -> CLI mode
    # 3. Default to web mode when no clear indication
    
    if args.serve and is_cli_command:
        print("Error: Cannot use --serve with CLI commands")
        sys.exit(1)
    
    if args.serve or (not is_cli_command and not args.command):
        # Web mode
        launch_web_server(args.host, args.port)
    elif is_cli_command:
        # CLI mode - pass control to CLI handler
        run_cli()
    else:
