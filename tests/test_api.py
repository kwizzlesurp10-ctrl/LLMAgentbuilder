...
def test_generate_agent_invalid_model():
    payload = {"name": "TestAgent", "prompt": "Test", "task": "Test", "model": "invalid-model", "provider": "anthropic"}
    response = client.post("/api/generate", json=payload)
...
"""
    payload = {"code": test_code, "task": "Hello World"}
...
def test_execute_agent_large_code():
    payload = {"code": "A" * 100001, "task": "Test"}  # Exceeds 100KB limit
    response = client.post("/api/execute", json=payload)
    assert response.status_code == 400
